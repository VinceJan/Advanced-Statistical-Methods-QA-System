...
- `deliverables/` 仅保存本地课程交付物；最终报告为 `高级统计方法知识图谱项目报告_重制版.docx`，已线下提交，不纳入公开代码仓库。旧 PPT 与演讲稿不属于当前仓库交付物，如需要可按脚本重新生成。
...
- 看到旧 PPT 或多个报告候选文件不要自动恢复；当前正式报告只保留本地 `deliverables/高级统计方法知识图谱项目报告_重制版.docx`，除非用户明确要求，否则不要把报告或旧 PPT 加回公开仓库。
